...
Simulation is done using function `rSnorm`. You can only simulate stationary Gaussian process with this function. For those unstationary processes but have stationary increments, you can first simulate their increments and then recover the target time series using function `cumsum`. We already prepare the ACF(or MSD, which can be easily converted into ACF using function `msd2acf`) for some common stationary Gaussain process and stationary increment process, like the fBm process increment, exponential decay process and matern process.
```{r}
require(SuperGauss)
