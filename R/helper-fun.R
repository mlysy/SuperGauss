...
## # forces x to be an N x p matrix or N x p x p array.
## # automatically converts if x is a scalar.
## # returns NA if there's a problem.
## .format.x <- function(x, N, p, type = c("matrix", "array")) {
##   type <- match.arg(type)
##   if(type == "matrix") {
##     if(is.vector(x)) {
##       if(length(x) == 1) {
...
##       }
##     }
##     if(!all(dims(x) == c(N, p))) x <- NA
##   } else {
##     if(is.vector(x)) {
##       if(length(x) == 1) {
##         # x is a scalar
##         x <- array(x, dim = c(N, p, p))
##       } else {
##         # x is a vector
##         x <- array(x, dim = c(length(x), 1, 1))
##       }
##     }
##     if(!all(dims(x) == c(N, p, p))) x <- NA
##   }
##   x
## }
