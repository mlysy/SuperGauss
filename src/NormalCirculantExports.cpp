...
///
/// @brief Creates an `Rcpp::XPtr` pointer to a NormalCirculant object and
/// passes the pointer address to the R session.
///
...
/// @param[in] calc_dldu Whether or to calculate the gradient with respect to
/// `uacf`.
///
...
//
// [[Rcpp::export]]
List NormalCirculant_grad_full(SEXP pNCt, NumericVector z, NumericVector uacf,
                               bool calc_dldz = true, bool calc_dldu = true) {
  XPtr<NormalCirculant> NCt(pNCt);
  int N = NCt->size();
  int Nu = N / 2 + 1;
  double ldens;
  ...
  ldens = NCt->grad_full(REAL(dldz), REAL(dldu), REAL(z), REAL(uacf), calc_dldz,
                         calc_dldu);
  List out;
  out["ldens"] = ldens;
  if (calc_dldz)
